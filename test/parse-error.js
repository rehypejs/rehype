...
// And then to `scriptingEnabled: false` not yet being supported.
...
test('parse-errors', function (t) {
  ...
  t.test('surrogate-in-input-stream', function (st) {
    var file = vfile({path: 'index.html', contents: '<!doctype html>\n' + String.fromCharCode(0xD800)});
  ...
  function next() {
    ...
    t.test(fixture, function (st) {
      ...
      var messages = JSON.parse(fs.readFileSync(path.join(fp, 'messages.json'), 'utf8'));
